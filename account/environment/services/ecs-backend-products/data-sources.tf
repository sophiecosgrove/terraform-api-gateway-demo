...
data "terraform_remote_state" "s3" {
  ...
  config = {
    bucket = "sophie-demo-terraform-state"
    key    = "terraform/account/environment/services/s3/terraform.tfstate"
    region = "eu-west-1"
  }
}
...
data "terraform_remote_state" "ecs_backend_users" {
  backend = "s3"
  config = {
    bucket = "sophie-demo-terraform-state"
...
data "terraform_remote_state" "dynamodb" {
  ...
  config = {
    bucket = "sophie-demo-terraform-state"
    key    = "terraform/account/environment/services/dynamodb/terraform.tfstate"
    region = "eu-west-1"
  }
}
